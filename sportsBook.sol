...
contract Sportsbook is owned
{
	...
	event BetPlaced(address indexed user, uint32 ticket, uint256 amount);
	event BetAccepted(address indexed user, uint32 ticket, string title);
	event BetRejected(address indexed user, uint32 ticket, string title, string reason);
	event BetGraded(address indexed user, uint32 ticket, string title, string result);
	event BetSettled(address indexed user, uint32 ticket);
	...
	function placeBet(uint64 slipid, uint32 ticket, string memory title) payable public {
        ...
        tempRakePool += slipid * globalRakePerMillion;
	...
	function placeBets(uint64 slipid, uint32[] tickets, string memory titles, uint256[] amounts, bool parlay) payable public {
	...
	function rejectBet(address user, uint32 ticket, string memory reason) public onlyAgent {
	...
	function refundBet(address user, uint32 ticket) public onlyAgent {
		uint256 betid = uniquebetid(user, ticket);
		Bet storage bet = bets[betid];
	...
	function acceptBet(address user, uint32 ticket, uint64 to_win) public onlyAgent {
		require(to_win != 0);
	...
	function gradeBet(address user, uint32 ticket, uint8 result, string memory reason) public onlyAgent {
		uint256 betid = uniquebetid(user, ticket);
		Bet storage bet = bets[betid];
	...
	function settleBet(address user, uint32 ticket) public onlyAgent {
		uint256 betid = uniquebetid(user, ticket);
		Bet storage bet = bets[betid];
	...
	function eraseBet(address user, uint32 ticket) public onlyAgent {
		uint256 betid = uniquebetid(user, ticket);
		Bet storage bet = bets[betid];
	...
	function eraseBets(address[] users, uint32[] tickets) public onlyAgent {
	...
	function uniquebetid (address user, uint32 ticket) private pure returns (uint256) {
